...
const numerosPunto3 = [12, 45, 68, 23, 89, 54, 31, 95, 10, 72];
const sumaTotalPunto3 = numerosPunto3.reduce(
...
const CANTIDAD_ALEATORIOS = 1500;
...
const sumaTotalAleatorios = numerosAleatorios.reduce(
  (acumulador, valorActual) => acumulador + valorActual,
  0
);
...
const promedioPunto3 = sumaTotalPunto3 / numerosPunto3.length;
...
const numerosMayoresA50 = numerosPunto3.filter(
  (numero) => numero > 50
);
...
function calcularPromedioEstudiante(estudiante) {
  const sumaNotas = estudiante.materias.reduce(
...
estudiantesAcademicos.forEach((estudiante, indice) => {
  const promedio = calcularPromedioEstudiante(estudiante);
  ...
  console.log(`      Materias: ${estudiante.materias.map((m) => `${m.nombre} (${m.nota})`).join(" | ")}`);
});

const promedioGeneralEstudiantes = sumaPromediosGlobal / estudiantesAcademicos.length;
...
const estudiantesConPromedioMayorA35 = estudiantesAcademicos.filter(
  (estudiante) => {
...
estudiantesConPromedioMayorA35.forEach((estudiante) => {
  const promedio = calcularPromedioEstudiante(estudiante);
